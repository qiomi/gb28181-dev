...
public:
    ...
    std::function<void(const std::string& call_id, int channel)>
        invite_callback_;
...
int SipStack::RespondInvite(const std::string& call_id, uint32_t ssrc,
    ...
    LOG_INFO("Responding to INVITE %s with SSRC %u, RTP port %d",
             call_id.c_str(), ssrc, rtp_port);
    return 0;
}
